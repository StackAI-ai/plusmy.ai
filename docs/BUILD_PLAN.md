...
11. Web product surface: ship onboarding, workspace switcher, member management, connection setup/callback pages, connection health/status, context library, prompt/skill editor, OAuth client approvals, audit log viewer, and MCP connection instructions; keep all write paths on Node runtime routes, not Edge.
