...
- 2026-03-21 execution checkpoint: tightened MCP client registration copy on the operator surface, added an explicit supported-client panel for OpenAI, Anthropic, Gemini, and Cursor on the MCP clients page, and split the remaining invite ownership-transfer edge case into follow-up `NIC-96`. Closed `NIC-26` after confirming connection sync job dedupe is already enforced in SQL/runtime.
...
- Continue with security/reliability test hardening in `NIC-19`, `NIC-20`, `NIC-22`, `NIC-25`, `NIC-27`, `NIC-28`, `NIC-29`, `NIC-33`, `NIC-37`, `NIC-38`, `NIC-39`, `NIC-40`, `NIC-41`, `NIC-42`, and `NIC-47`; these still need a real DB/test harness or targeted implementation before they can be closed confidently.
- The invite acceptance flow now has a dedicated ownership-transfer follow-up in `NIC-96`; keep `NIC-41` narrowly focused on expired/duplicate invite UX.
